...
You might be wondering, what's going on? There isn't an actual package named `C` in the standard library, `C` is an automatically generated package whose identifiers come from the C code in the comments that automatically precede it. so if you declare a C function named `hello`, Cgo makes it available in your Go program as `C.hello`. you could also invoke functions, and reference global variables that are imported into the comment block from C libraries via header files, you just have to make sure the file is in the same directory as your Go code.
